...
setup_zshrc() {
  # Keep most recent old .zshrc at .zshrc.pre-dots, and older ones
  # with datestamp of installation that moved them aside, so we never actually
  # destroy a user's original zshrc
  echo "${BLUE}Looking for an existing zsh config...${RESET}"

  # Must use this exact name so uninstall.sh can find it
  OLD_ZSHRC=~/.zshrc.pre-dots
  if [ -f ~/.zshrc ] || [ -h ~/.zshrc ]; then
    ...
    if [ -e "$OLD_ZSHRC" ]; then
      ...
      echo "${YELLOW}Found old ~/.zshrc.pre-dots." \
  ...
  echo "${GREEN}Using the Dots template file and adding it to ~/.zshrc.${RESET}"
...
EOF
  ...
  if [ -n "$SHELL" ]; then
    echo "$SHELL" > ~/.shell.pre-dots
  else
    grep "^$USERNAME:" /etc/passwd | awk -F: '{print $7}' > ~/.shell.pre-dots
  fi
