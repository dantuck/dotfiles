...
main :: IO ()
main = do
	spawn "/home/daniel/.xmonad/bin/cpuUsage.sh 0"
	spawn "/home/daniel/.xmonad/bin/cpuUsage.sh 1"
	spawn "/home/daniel/.xmonad/bin/cpuUsage.sh 2"
	spawn "/home/daniel/.xmonad/bin/cpuUsage.sh 3"
	spawn "/home/daniel/.xmonad/bin/jobs.rb"
...
myIconPath           = "/home/daniel/.xmonad/.icons/xbm_icons/subtle/"
myBinPath            = "/home/daniel/.xmonad/bin/"
xRes                 = 1366
yRes                 = 768
panelHeight          = 16
panelBoxHeight       = 12
...
batPercent :: Logger
batPercent = do
	percent <- liftIO $ readWithE "/sys/class/power_supply/BAT1/capacity" "N/A" "%"
	return $ return percent

batStatus :: Logger
batStatus = do
	status <- liftIO $ readWithE "/sys/class/power_supply/BAT1/status" "AC Conection" ""
...
-- TopLeftBar
myTopLeftBar = "dzen2 -x 0 -y 0 -h 16 -w 850 -ta 'l' -fg '" ++ colorWhiteAlt ++ "' -bg '" ++ colorBlack ++ "' -fn '" ++ dzenFont ++ "' -p -e 'onstart=lower'"
myLogHook2 :: Handle -> X ()
...
-- TopRightBar
myTopRightBar = "dzen2 -x 850 -y 0 -h 16 -w 566 -ta 'r' -fg '" ++ colorWhiteAlt ++ "' -bg '" ++ colorBlack ++ "' -fn '" ++ dzenFont ++ "' -p -e 'onstart=lower'"
myLogHook3 :: Handle -> X ()
myLogHook3 h = dynamicLogWithPP $ defaultPP
	{ ppOutput          = hPutStrLn h
    , ppOrder           = \(_:_:_:x) -> x
	, ppSep             = " "
	, ppExtras          = [ date $ (wrapTextBox colorBlack colorWhiteAlt colorBlack "%A") ++ (wrapTextBox colorWhiteAlt colorGrayAlt colorBlack $ "%Y^fg(" ++ colorGray ++ ").^fg()%m^fg(" ++ colorGray ++ ").^fg(" ++ colorBlue ++ ")%d ^fg(" ++ colorGray ++ ")-^fg() %H^fg(" ++ colorGray ++"):^fg()%M^fg(" ++ colorGray ++ "):^fg()^fg(" ++ colorGreen ++ ")%S^fg()")
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
  ...
  , ((modMask, xK_p),
	...
	--, ((modMask .|. shiftMask, xK_c), spawn "finch")
	--, ((modMask .|. shiftMask, xK_c), spawn "urxvt -name mcabber -e mcabber")
  ...
  , ((0, xF86XK_MonBrightnessUp), spawn $ myBinPath ++ "bridzen.sh + -d")                                                    --Raise brightness
	, ((0, xF86XK_MonBrightnessDown), spawn $ myBinPath ++ "bridzen.sh - -d")                                                  --Lower brightness
